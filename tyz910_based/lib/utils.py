...
def read_csv(file_name, nrows):
    ...
    logf('file {}'.format(file_name))
    df = pd.read_csv(file_name, low_memory=False, nrows=nrows)
    logf('dataset shape: {}, nrows: {})'.format(df.shape, nrows))
    return df
...
def optimize_dataframe(df):
    ...
    logf('optimize dataframe ({} to {}, ratio: {})'.format(old_size, new_size, round(old_size/new_size, 2)))
